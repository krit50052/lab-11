...
int main(){	
	...
	if(playerAction == 1){
		playerCards[2] = drawCard(); // สุ่มไพ่ใบที่ 3
  		playerScore = calScore(playerCards[0], playerCards[1], playerCards[2]); // คำนวณคะแนนใหม่
	...
	yugiAction = findYugiAction(yugiScore); // ใส่ yugiScore เป็น parameter
	
	if(yugiAction == 1){
		yugiCards[2] = drawCard(); // สุ่มไพ่ใบที่ 3 ให้ Yugi
		yugiScore = calScore(yugiCards[0], yugiCards[1], yugiCards[2]); // คำนวณคะแนนใหม่
